...
class FastSCNN(nn.Module):
    def __init__(self, num_classes, aux=True, **kwargs):
        ...
        self.classifier = Classifer(128, num_classes[0])
        self.bgclassifier = Classifer(128, num_classes[1])
        if self.aux:
            self.auxlayer = nn.Sequential(
                ...
                nn.ReLU(True),
                nn.Dropout(0.1),
                nn.Conv2d(32, sum(num_classes), 1)
            )
    def init_output(self, n_sigma=1):
        with torch.no_grad():
            output_conv = self.classifier.output_conv
    ...
    def forward(self, x):
        ...
        x = torch.cat([self.classifier(x), self.bgclassifier(x)], 1)
        outputs = []
...
class Classifer(nn.Module):
    ...
    def __init__(self, dw_channels, num_classes, stride=1, **kwargs):
        ...
        self.output_conv = nn.Conv2d(dw_channels, num_classes, 1)
        self.conv = nn.Sequential(
            nn.Dropout(0.1),
            self.output_conv
        )
